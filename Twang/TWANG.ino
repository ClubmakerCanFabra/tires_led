...
#define LEFT_PIN   8
#define RIGHT_PIN  9
#define ATTACK_PIN 10
unsigned long currentTime, pressedTime;
#define KEY_TIME 40
...
void setup() {
    ...
    // Fast LED
    FastLED.addLeds<WS2811, DATA_PIN, GRB>(leds, NUM_LEDS);   //Try BGR or GBR
...
void tickBoss() {
    ...
    if(boss.Alive()) {
        ...
        // CHECK FOR ATTACK
        if(attacking) {
            if( (getLED(playerPosition+(ATTACK_WIDTH/2)) >= getLED(boss._pos - BOSS_WIDTH/2) && getLED(playerPosition+(ATTACK_WIDTH/2)) <= getLED(boss._pos + BOSS_WIDTH/2)) ||
...
void getInput() {
    ...
    if(digitalRead(LEFT_PIN) == LOW) {
        if(currentTime-pressedTime > KEY_TIME) {
            joystickTilt -= 10;
            if (abs(joystickTilt) > 90) joystickTilt = -90;
    ...
    if(digitalRead(ATTACK_PIN) == LOW) joystickWobble = ATTACK_THRESHOLD+1;
    else joystickWobble = ATTACK_THRESHOLD-1;
}
...
void SFXtilt(int amount) { 
    ...
    //toneAC(freq, min(min(abs(amount)/9, 5), MAX_VOLUME));
    tone(MUSIC_PIN, freq);
